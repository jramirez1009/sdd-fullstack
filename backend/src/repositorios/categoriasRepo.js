...
// en el controlador: una categoría ajena sencillamente no se encuentra, de modo
...
/** Todas las categorías del usuario, ordenadas por nombre. */
...
/**
 * Inserta una categoría del usuario. La violación de la restricción UNIQUE
 ...
 */
export async function insertarCategoria({ usuarioId, nombre }) {
  const { rows } = await consultar(
...
/** Renombra una categoría del usuario. Devuelve null si no hay tal fila suya. */
export async function actualizarCategoria({ id, usuarioId, nombre }) {
  const { rows } = await consultar(
    `UPDATE categorias
     SET nombre = $1
...
/** Elimina una categoría del usuario. Devuelve si se eliminó alguna fila. */
